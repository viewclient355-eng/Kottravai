...
async function run() {
    ...
    let res = await sheets.spreadsheets.values.get({ spreadsheetId, range: "'Dashboard'!A1:F15" });
    console.log('--- DASHBOARD ---');
    console.table(res.data.values);

    let res2 = await sheets.spreadsheets.values.get({ spreadsheetId, range: "'Executive Summary'!A1:H10" });
    console.log('\n--- EXECUTIVE SUMMARY ---');
